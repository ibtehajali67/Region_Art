...
def segment(image):
    ...
    return regions.clip(0, 255).astype(np.uint8)
...
if __name__=='__main__':
    ...
    for i in os.listdir(Dir_1):
        x=os.path.join(Dir_1, i)
        image = cv2.imread(x)
        region = segment(image)
        #cv2.imwrite('./6/'+str(count)+'current_skeleton.png', skeleton)
        cv2.imwrite(path + str(count)+'current_region.png', region)
        #cv2.imwrite('./8/'+str(count)+'current_flatten.png', flatten)
        count+=1
